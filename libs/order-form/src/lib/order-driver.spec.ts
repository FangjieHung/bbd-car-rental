...
import { DriverCredential, calculatePrice } from '@car-rental/domain';
...
describe('填了駕照號碼時標準化車種必填', () => {
  it('送出前的檢查：留空不擋；填了號碼沒選車種＝承租人步驟的問題', () => {
    const form = createOrderForm();
    expect(orderFormProblems(form.getRawValue(), noProblemsDerived(), TEST_ORDER_FORM_LABELS).renter).not.toContain(
      t.orderForm.problems.driverClassRequired,
    );
    form.controls.driver.patchValue({ licenseNumber: 'TL-1' });
    expect(orderFormProblems(form.getRawValue(), noProblemsDerived(), TEST_ORDER_FORM_LABELS).renter).toContain(
      t.orderForm.problems.driverClassRequired,
    );
