...
import { RentalOrder, Vehicle } from '../models';
...
function order(p: Partial<RentalOrder> = {}): RentalOrder {
  return {
    ...
    pickupBranchId: '', returnBranchId: '', status: 'reserved', depositRequired: 0, ...p,
  };
}

/** 1/6 09:00 – 1/7 09:00，與上面預設訂單（1/5–1/8）重疊。 */
const period = (orders: RentalOrder[], excludeBookingId?: string) => ({
...
describe('vehicleUnavailableReasons（可租判斷的唯一來源）', () => {
  ...
  it('已預訂、出租中的重疊訂單都佔用車輛，原因帶出那筆訂單', () => {
    const reserved = order({ id: 'r' });
    const inProgress = order({ id: 'p', status: 'in_progress' });
    expect(vehicleUnavailableReasons(vehicle(), period([reserved]))).toEqual([{ kind: 'booked', order: reserved }]);
    expect(vehicleUnavailableReasons(vehicle(), period([inProgress]))).toEqual([{ kind: 'booked', order: inProgress }]);
  });
  ...
  it('已取消、已完成、別台車、無縫接續的訂單都不佔用', () => {
    const orders = [
      ...
      order({ id: 'o', vehicleId: 'v2' }),
      // 1/7 09:00 取車，剛好接在查詢期間之後
      order({ id: 'n', startTime: iso('2026-01-07T09:00'), endTime: iso('2026-01-09T09:00') }),
    ];
    expect(vehicleUnavailableReasons(vehicle(), period(orders))).toEqual([]);
  });

  it('排除這筆訂單自己（編輯訂單時）→ 自己的車不算被佔用', () => {
    expect(vehicleUnavailableReasons(vehicle(), period([order({ id: 'self' })], 'self'))).toEqual([]);
  });

  it('同時保養中又有多筆重疊訂單：全部列出，訂單依取車時間排序', () => {
    const later = order({ id: 'later', startTime: iso('2026-01-06T12:00'), endTime: iso('2026-01-06T18:00') });
    const earlier = order({ id: 'earlier', startTime: iso('2026-01-04T09:00'), endTime: iso('2026-01-06T10:00') });
    expect(vehicleUnavailableReasons(vehicle({ status: 'maintenance' }), period([later, earlier]))).toEqual([
      { kind: 'maintenance' },
      { kind: 'booked', order: earlier },
      { kind: 'booked', order: later },
    ]);
  });
});
...
describe('vehicleAvailability', () => {
  it('分成可以租／不能租兩組，各自維持傳入順序', () => {
    ...
    expect(result.unavailable).toEqual([
      ...
      { vehicle: v3, reasons: [{ kind: 'booked', order: b }] },
    ]);
  });

  it('與 isVehicleAvailable 永遠同一個結果', () => {
    const vehicles = [vehicle({ id: 'v1' }), vehicle({ id: 'v2', status: 'maintenance' }), vehicle({ id: 'v3' })];
    const orders = [order({ vehicleId: 'v3' }), order({ id: 'b2', vehicleId: 'v1', status: 'cancelled' })];
    const p = period(orders);
    const available = vehicleAvailability(vehicles, p).available.map((v) => v.id);
    const byOldCheck = vehicles.filter((v) => isVehicleAvailable({ vehicle: v, ...p, orders })).map((v) => v.id);
