...
export const appConfig: ApplicationConfig = {
  providers: [
    ...
    provideAppInitializer(() => inject(ThemeService).init()),
    {
      ...
      useFactory: () => new LocalStorageRepository('cr.vehicles', seedVehicles, notifyStorageReset(inject(MatSnackBar))),
    },
    {
      provide: CUSTOMER_REPO,
      useFactory: () => new LocalStorageRepository('cr.customers', seedCustomers, notifyStorageReset(inject(MatSnackBar))),
    },
    {
      provide: BOOKING_REPO,
      useFactory: () => new LocalStorageRepository('cr.bookings', seedBookings, notifyStorageReset(inject(MatSnackBar))),
    },
    {
      provide: MAINTENANCE_REPO,
      useFactory: () =>
        new LocalStorageRepository('cr.maintenance', seedMaintenanceRecords, notifyStorageReset(inject(MatSnackBar))),
    },
  ],
};
