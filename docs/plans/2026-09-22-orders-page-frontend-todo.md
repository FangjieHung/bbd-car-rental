...
- **選保險方案頁目前不可達**：`libs/booking-flow/src/lib/pages/plan-page.component.ts` 存在但沒有被任何 app 的路由掛上，是在 2026-09-18 的 `417493a`（`wip: …保費相關欄位回退…`）被退掉的，那筆 wip 至今沒有收尾。後果是**官網下單流程的保險費永遠是 0**。要嘛把它接回路由，要嘛確認這個功能確定不要並把元件刪掉——現在這樣是最糟的狀態（程式在、沒人用、看文件的人會以為有）。
