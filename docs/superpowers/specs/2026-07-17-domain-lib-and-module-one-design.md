...
```ts
// 狀態機擴充：新增 pending_payment 待付款中間狀態
// 生命週期：pending_payment → confirmed → in_progress → completed（或任一階段 cancelled）
// pending_payment：前台送出後、款項尚未確認。金流接上後由 webhook 自動轉 confirmed；
//                  現金/轉帳則由 admin 人工確認後轉 confirmed。本輪無金流，一律停在此狀態等人工確認。
...
// on_site=現場付現/刷卡；bank_transfer=ATM/轉帳（皆需人工確認）
...
### 3.4 車輛可租判定（時間重疊檢查，純函式）

前台「選車款」不能只看 `Vehicle.status`，必須排除該租期已被其他訂單佔用的車。放 libs/domain：

```ts
// 兩租期 [s1,e1) 與 [s2,e2) 重疊 ⇔ s1 < e2 && s2 < e1
export function rangesOverlap(s1: string, e1: string, s2: string, e2: string): boolean;

// 某車在指定租期是否可租：狀態非 maintenance，且無「未取消/未完成」訂單與該租期重疊
export function isVehicleAvailable(input: {
  vehicle: Vehicle;
  startTime: string; endTime: string;
  bookings: RentalBooking[];        // 該車所有既有訂單
}): boolean;
```

判定規則：
- `vehicle.status === 'maintenance'` → 不可租。
- 佔用訂單＝status 為 `pending_payment | confirmed | in_progress` 者（`completed`、`cancelled` 不佔用）。
- 任一佔用訂單與請求租期 `rangesOverlap` → 不可租。
- 邊界：前一單的 `endTime` 等於後一單的 `startTime`（無縫接續）視為不重疊、可租（半開區間 `[start, end)`）。

前台步驟 2 用 `isVehicleAvailable` 過濾車輛清單，取代單純看 status。
...
- **訂單頁人工確認**：既有 bookings 頁加入把 `pending_payment` 訂單「確認收款」轉 `confirmed` 的動作（現金/轉帳人工確認用；經 BookingStore，不改狀態機以外的流程）。
...
1. **選租期**：起訖日期 + 時間。
2. **選車款**：用 `isVehicleAvailable`（§3.4 時間重疊檢查）過濾出該租期真正可租的車輛，依 category 分組，呼叫 `calculatePrice` 即時預覽各車款租金。不是只看 `status`，而是排除該租期已被 `pending_payment/confirmed/in_progress` 訂單佔用的車。
3. **選配件**：勾選 `AddOn` 與數量。
...
6. **送出**：寫一筆 `RentalBooking`（status=`pending_payment`，帶 addOns/couponCode/priceBreakdown/paymentMethod）進 localStorage，並在需要時建立/關聯 `Customer`。顯示完成頁（提示「訂單成立，待付款/待人工確認」）。無金流。後續由 admin 端人工把 `pending_payment` 轉 `confirmed`（現金/轉帳確認），或未來金流 webhook 自動轉。送出前需再次以 `isVehicleAvailable` 覆核，避免併發下同車重複預約。
...
libs/domain/src/
  ...
  lib/availability/              # is-vehicle-available.ts + ranges-overlap.ts（純函式）
...
| `isVehicleAvailable` / `rangesOverlap` | 純函式單元測試：重疊/不重疊、無縫接續（前單 end===後單 start 視為可租）、maintenance 車不可租、completed/cancelled 訂單不佔用、pending_payment/confirmed/in_progress 佔用 |
...
4. booking 前台可從選租期一路走到明細確認頁：選車款只列出該租期可租的車（時間重疊檢查生效），金額與 admin 設定的定價/配件/優惠券一致，送出後 localStorage 多一筆 `pending_payment` booking；admin 訂單頁可將其人工確認轉 `confirmed`。
