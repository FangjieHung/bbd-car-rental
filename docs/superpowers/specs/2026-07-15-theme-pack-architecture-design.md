# 雙軸主題系統設計（質地 Paradigm × 配色 Color-theme）
...
一個畫面的外觀 = **選一個質地 ＋ 選一個配色**。兩軸獨立、可自由組合：
...
N 個質地 ＋ M 個配色 = N×M 種樣子，只維護 N＋M 份。技術上就是最外層兩個獨立屬性：
...
| **質地包** paradigm（可獨立換） | **管造型結構** | button/card/table/status/typography 的樣式規則 ＋ 造型 token（圓角/陰影/模糊/光邊/字體） | Material、Glass、Neumo |

同一配色可套不同質地；同一質地可換不同配色。Material+Verdant 與 Material+Midnight 只差在顏色，
造型（圓角/陰影/字體）一樣。
...
| 決策 | 選定 |
|---|---|
| 主題結構 | 二維：質地 × 配色，兩獨立屬性切換 |
| Token 詞彙 | Material 3 sys token（`--mat-sys-*`）為單一詞彙表；顏色值來自配色包、造型值來自質地包 |
| 間距 | 交給 Tailwind（`p-4`/`gap-3`，4px 網格＝對齊 M3 4dp），不進 CSS 變數 |
| 元件樣式歸屬 | 每質地一份（因跨質地結構本質不同）；但顏色/字體仍綁 token |
| 換膚時機 | runtime 即時切（兩軸各一個下拉）＋ project-time 可鎖 |
| 第一個計畫範圍 | 雙軸結構就位；質地軸先 Material 一個；配色軸做 Verdant＋Midnight 兩個 |

---

## 3. 契約：從「token 名」升級成「class 名單」（CSS Zen Garden 模型）

因為元件樣式每質地一份，契約不只是 token，而是**骨架 HTML 用的一組固定 class**。
每個質地包必須實作這同一組 class；骨架的 HTML 與功能永不動，變的是「哪一包 CSS 認這些 class」。
如同 CSS Zen Garden：同一份 HTML，套不同 CSS，就是另一個世界。

Class 契約（初版，實作時定稿並寫進 `_contract.scss`）。**命名慣例**：
- **前綴中性**：元件一律 `ui-`（代表設計系統元件，不綁配色/質地）；暫時狀態用 `is-`。
...
造型 token（corner-*/level*/字體）由質地包給值。Glass/Neumo 需要 M3 沒有的東西
（`backdrop-filter`、`inset` 陰影）——那些不是 token，是質地 recipe 裡的 CSS。
...
src/styles/
  ...
  texture/                       // 【質地軸】= 造型（recipe + 造型 token）
...
src/app/core/theme/
  ...
  theme.token.ts                   // 質地清單 + 配色清單常數（id + 顯示名）
  status-tone.ts                   // 狀態→tone 單一真相源（見 §5）
src/app/shared/
  theme-switcher.component.*        // 兩個下拉：質地、配色（project-time 可刪）
...
- **tone 的造型**（chip 圓角、是否有 dot、邊框）= **質地包**的 `status.scss`。
...
- `--font-en`：英文依**質地**換（Material 用一種、Glass/Neumo 各自風格）。
- `typography.scss`（質地包內）定義 `.v-text-{display,title,heading,body,label,caption}` 綁 M3 type token
...
把兩軸想成 Figma 的**元件庫（質地）× 變數 Mode（配色）**。換膚＝在最外層改 `data-paradigm` /
...
複製到新專案要收斂時：
1. `ThemeService` 預設值設死要用的質地＋配色（或直接寫死 `<html>` 兩屬性）。
2. （可選）刪換膚器元件與外殼引用。
3. （可選）刪用不到的 `texture/*`、`color-themes/*` 資料夾，各自 `_registry.scss` 移除 `@use`。
...
**只排除 `src/styles/color-themes/**`**（顏色值的唯一來源）與各質地的 `_tokens.scss`（造型 token 定義處）。
其餘地方若出現十六進位色碼（`#xxx`/`#xxxxxx`）或底層色階變數，即 **exit 1** 並列檔案:行號。
（`--mat-sys-*` 與 `--app-*` 允許。質地 recipe 只能用 token 表達顏色，不得寫死。）納入 CI / commit 前檢查。

---

## 10. 第一個計畫要做出的東西（範圍清單）

1. 檔案結構 §4：`texture/material/`（5 個 recipe + `_tokens.scss`）、`color-themes/{verdant,midnight}/_tokens.scss`、兩個 `_registry.scss`、精簡 `styles.scss`、`_skeleton.scss`、`_contract.scss`。
2. 把現有 `styles.scss` 自創語意層拆解：顏色 → 配色包；造型 → material 質地 `_tokens`；元件樣式 → material 質地 recipe。
...
| 現有 `styles.scss` 自創語意層 | `--surface-*`/`--text-*`/`--radius-*`… | 拆進配色包 / material 質地（見 §10.2） |

修完後 `color-themes/` 與質地 `_tokens.scss` 以外應為 **0 處** 底層色/寫死色。

---

## 12. Midnight 配色包規格（證明配色軸可換）

只覆寫**顏色** token（造型沿用 Material 質地，不動）：
...
目標：`data-theme` 切 verdant↔midnight（質地維持 material），**只有顏色變、版面完全不動**。
...
3. **如何新增一個質地**：複製 `texture/material/` → 依 §3 class 契約重寫 recipe（可用 `backdrop-filter` 等自由 CSS）→ 註冊。
4. **鎖成單一主題（初始化步驟）**：即 §8。
...
4. **配色軸**：質地固定 material，`data-theme` 切 verdant↔midnight，vehicles/bookings/dashboard 三頁、桌機＋390px 目視：版面一致、只有配色變、無破圖、對比足夠。
5. **質地軸插槽**：`data-paradigm` 屬性機制就位，切換器可選（雖只有 material 一個）；`_registry.scss` 結構可直接加第二個質地。
...
- 本次**不做** Glass / Neumorphism 質地（質地軸只做 Material，插槽預留）。
...
本次 class 契約（§3）已為此鋪路——骨架用 `.v-*` 契約 class，未來質地可完全接管其樣式。
