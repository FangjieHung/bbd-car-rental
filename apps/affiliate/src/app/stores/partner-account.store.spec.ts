...
function setup(bookings: RentalOrder[], payouts: MonthlyPayout[] = [], partners: Partner[] = [partner, perDayPartner]) {
...
describe('PartnerAccountStore', () => {
  ...
  it('只列出 sourcePartnerId 相符的訂單，並用 percent 佣金規則算出每筆與合計', () => {
    const store = setup([
      makeOrder({ id: 'b1', sourcePartnerId: 'pt1' }),
      makeOrder({ id: 'b2', sourcePartnerId: 'pt2' }),
      makeOrder({ id: 'b3', sourcePartnerId: undefined }),
    ]);
  ...
  it('多筆訂單佣金加總為累計合計', () => {
    const store = setup([
      makeOrder({ id: 'b1', sourcePartnerId: 'pt1' }),
      makeOrder({ id: 'b2', sourcePartnerId: 'pt1' }),
    ]);
    const account = store.getAccount('seaview');
  ...
  it('有撥款紀錄的月份顯示該紀錄的狀態', () => {
    const store = setup(
      [makeOrder({ id: 'b1', sourcePartnerId: 'pt1', startTime: '2026-07-05T09:00:00' })],
