...
export class OrderStore {
  private repo = inject(ORDER_REPO);
  private vehicleStore = inject(VehicleStore);

  private _orders = signal<RentalOrder[]>(this.repo.getAll());
  readonly orders: Signal<RentalOrder[]> = this._orders.asReadonly();
  ...
  create(input: Omit<RentalOrder, 'id' | 'status'>): RentalOrder {
    this.validate(input.vehicleId, input.startTime, input.endTime);
    const order: RentalOrder = { id: crypto.randomUUID(), ...input, status: 'reserved' };
    this.repo.create(order);
    this.reload();
    return order;
  }

  updateOrder(id: string, patch: Partial<Omit<RentalOrder, 'id' | 'status'>>): void {
  ...
  pickUp(id: string): void {
    ...
    if (b.status !== 'reserved') throw new Error(ZH_TW.order.invalidTransition);
  ...
  complete(id: string): void {
    const b = this.mustGet(id);
    if (b.status !== 'in_progress') throw new Error(ZH_TW.order.invalidTransition);
  ...
  cancel(id: string): void {
    ...
    if (b.status !== 'reserved') {
      throw new Error(ZH_TW.order.invalidTransition);
    }
  ...
  private validate(vehicleId: string, startIso: string, endIso: string, excludeId?: string): void {
    if (new Date(endIso) <= new Date(startIso)) throw new Error(ZH_TW.order.endBeforeStart);
    const conflicts = this.findConflicts(vehicleId, startIso, endIso, excludeId);
    if (conflicts.length > 0) {
      throw new Error(`${ZH_TW.order.conflict} ${conflicts.map((c) => c.id).join(', ')}`);
    }
  }
  ...
  private mustGet(id: string): RentalOrder {
  ...
  private reload(): void {
    this._orders.set(this.repo.getAll());
  }
}
