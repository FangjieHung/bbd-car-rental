 ...
 *
 * 取車／還車完成後，order() 的狀態會透過 OrderStore 的 signal 自動反映，畫面因此
...
export class HandoverPanelComponent {
  ...
  protected readonly order = computed(() => this.orderStore.orders().find((b) => b.id === this.bookingId()));
  protected readonly vehicle = computed(() => {
    const order = this.order();
    return order ? this.vehicleStore.vehicles().find((v) => v.id === order.vehicleId) : undefined;
  });
  protected readonly member = computed(() => {
    const order = this.order();
    return order ? this.memberStore.members().find((m) => m.id === order.memberId) : undefined;
  });
   ...
   */
  protected readonly previousRental = computed<RentalOrder | undefined>(() => {
    const order = this.order();
  ...
  protected readonly readinessInput = computed<PickupReadinessInput | undefined>(() => {
    ...
    if (!order || !vehicle) return undefined;
  ...
  protected async submitPickup(): Promise<void> {
    ...
    if (!order || !input || this.pickupForm.invalid) return;
  ...
  protected previewReturnCharges(): void {
    ...
    if (!order || !vehicle || !pickup) return;
    if (
    ...
    const policy = this.policyFor(order, vehicle);
   ...
   * 不阻擋還車、也不會平白多收錢。
   */
  private policyFor(order: RentalOrder, vehicle: Vehicle) {
