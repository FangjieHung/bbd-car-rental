...
export class ActivityTimelineComponent {
  ...
  private paymentEvents(bookingId: string): ActivityEvent[] {
    return this.paymentStore.paymentsFor(bookingId).map((p) => ({
      ...
      title: `${this.t.activityTimeline.kindLabels['payment']}：${p.amount} 元（${this.t.orderForm.paymentMethodLabels[p.method] ?? p.method}）`,
  ...
  private refundAndCreditEvents(
    ...
    for (const r of this.paymentStore.refundsFor(bookingId)) {
      ...
      events.push({
        ...
        at,
        kind: 'refund',
        title: `${this.t.activityTimeline.kindLabels['refund']}：${r.amount} 元（${this.t.orderForm.paymentMethodLabels[r.method] ?? r.method}）`,
