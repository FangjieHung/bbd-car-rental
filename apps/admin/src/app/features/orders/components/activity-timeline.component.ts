...
export class ActivityTimelineComponent {
  ...
  protected readonly events = computed<ActivityEvent[]>(() => {
    ...
    if (order) {
      events.push(...this.documentEvents(order.memberId));
    }

    const cancellationCaseIds = this.cancellationStore.casesFor(bookingId).map((c) => c.id);
    events.push(...this.refundAndCreditEvents(bookingId, order?.memberId, cancellationCaseIds));
  ...
  private paymentEvents(bookingId: string): ActivityEvent[] {
    return this.paymentStore.paymentsFor(bookingId).map((p) => ({
      ...
      title: `${this.t.activityTimeline.kindLabels['payment']}：${formatTwd(p.amount)}（${this.t.orderForm.paymentMethodLabels[p.method] ?? p.method}）`,
      detail: `${this.t.orderForm.paymentPurposeLabels[p.purpose] ?? p.purpose} · ${this.t.paymentPanel.recordStatusLabels[p.status] ?? p.status}`,
      actor: p.handledBy,
    }));
  }
  ...
  private refundAndCreditEvents(
    ...
    for (const r of this.paymentStore.refundsFor(bookingId)) {
      ...
      events.push({
        ...
        at,
        kind: 'refund',
        title: `${this.t.activityTimeline.kindLabels['refund']}：${formatTwd(r.amount)}（${this.t.orderForm.paymentMethodLabels[r.method] ?? r.method}）`,
