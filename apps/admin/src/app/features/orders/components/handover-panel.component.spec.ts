...
describe('HandoverPanelComponent', () => {
      ...
      providers: [
        { provide: VEHICLE_REPO, useValue: createInMemoryRepo<Vehicle>([makeVehicle(options.vehicle)]) },
        {
          ...
          useValue: createInMemoryRepo<RentalOrder>([
            makeBooking({ depositRequired: options.depositRequired ?? 0, ...options.order }),
  ...
  it('訂金未達門檻但填了完整主管覆核：可以成功取車，訂單轉為 in_progress', async () => {
    ...
    const orderStore = TestBed.inject(OrderStore);
  ...
  describe('還車', () => {
     ...
     * 讓 order().status 反應成 in_progress 後畫面自動切到還車表單，而不是另外
     * 繞過元件直接呼叫 orderStore.pickUp()（那樣不會留下 HandoverRecord）。
     */
    async function createFixtureAfterPickup(pickupOverrides: { mileage?: number; energyLevel?: number } = {}) {
      ...
      await fixture.componentInstance['submitPickup']();
      fixture.detectChanges();
      expect(TestBed.inject(OrderStore).orders()[0].status).toBe('in_progress');
      return fixture;
    }
  ...
  describe('還車提醒狀態（Task 17）', () => {
    ...
    it('沒有任何提醒紀錄時不顯示提醒區塊', () => {
      configure({ order: { status: 'in_progress' } });
      const fixture = createFixture();
      const el = fixture.nativeElement as HTMLElement;
