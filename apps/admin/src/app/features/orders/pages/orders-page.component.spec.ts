...
describe('OrdersPageComponent filtering', () => {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: createInMemoryRepo<RentalOrder>([
            makeOrder({ id: 'b1', vehicleId: 'v1', memberId: 'c1', status: 'reserved' }),
            makeOrder({ id: 'b2', vehicleId: 'v2', memberId: 'c2', status: 'in_progress' }),
            makeOrder({ id: 'b3', vehicleId: 'v1', memberId: 'c2', status: 'cancelled' }),
          ]),
        },
  ...
  it('依會員姓名搜尋', () => {
    ...
    expect(component.filteredOrders().map((b) => b.id)).toEqual(['b1']);
  });

  it('依車牌搜尋（不分大小寫）', () => {
    component.searchQuery.set('xyz');
    expect(component.filteredOrders().map((b) => b.id)).toEqual(['b2']);
  });

  it('依訂單狀態篩選', () => {
    component.statusFilter.set('cancelled');
    expect(component.filteredOrders().map((b) => b.id)).toEqual(['b3']);
  });
...
describe('OrdersPageComponent 急迫指標與排序', () => {
  ...
  function setup() {
    ...
    const ordinary = makeOrder({ id: 'ordinary', status: 'reserved' });
    // overdueReturn：in_progress 且還車時間已過，屬於逾時未還。
    const overdueReturn = makeOrder({
      id: 'overdue-return',
      status: 'in_progress',
      endTime: new Date(2026, 7, 10, 9).toISOString(),
    });
    // refundPending：cancelled 訂單，有一筆狀態 pending 的退款紀錄。
    const refundPending = makeOrder({ id: 'refund-pending', status: 'cancelled' });
    // recovering：reserved 訂單，有一筆 in_progress 的業者復原案件。
    const recovering = makeOrder({ id: 'recovering', status: 'reserved' });
  ...
  it('急迫列排在一般列之前，且維持原始載入順序（穩定排序）', () => {
    ...
    expect(component.filteredOrders().map((b) => b.id)).toEqual([
      'overdue-return',
      'refund-pending',
      'recovering',
      'ordinary',
