...
import { ChargeAdjustment, MaintenanceRecord, PaymentRecord, RefundRecord, RentalBooking, Repository, Vehicle } from '../../../core/models';
...
function createFixture(options: FixtureOptions = {}) {
  ...
  const bookingRepo = createInMemoryRepo<RentalBooking>([booking]);
  ...
  fixture.componentRef.setInput('bookingId', booking.id);
  fixture.detectChanges();
  const paymentStore = TestBed.inject(PaymentStore);

  return { fixture, component: fixture.componentInstance, booking, bookingRepo, paymentRepo, refundRepo, adjustmentRepo, paymentStore };
}
...
describe('PaymentPanelComponent 摘要區塊', () => {
  ...
  it('混合付款：兩種不同付款方式時，方式摘要顯示「混合付款」而不是被其中一個覆蓋', () => {
    ...
    expect(component['methodSummaryLabel']()).toBe(component['t'].bookingForm.paymentMethodLabels['cash']);
