        ...
        <div class="grid" [style.grid-template-columns]="gridCols">
          ...
          @for (d of days(); track $index) {
            <div
              ...
              style="color: var(--mat-sys-on-surface-variant); border-left: 1px solid var(--mat-sys-outline-variant)"
            >
              {{ fmtDate(d) }}
            </div>
          }
              ...
              <div class="text-sm p-2 whitespace-nowrap">
                ...
                @if (v.status === 'maintenance') {
                  <span class="text-xs" style="color: var(--mat-sys-on-surface-variant)"
                    >（{{ t.dispatch.maintenanceBlock }}）</span
                  >
                }
              </div>
              @for (d of days(); track $index) {
                <div
                  ...
                  [style.background]="
                    v.status === 'maintenance' && $index === todayIdx() ? 'var(--app-viz-4)' : null
                  "
                ></div>
              }
            </div>
            ...
            >
              ...
              @for (block of blocksOf(v.id); track block.bookingId) {
                <button
                  ...
                  [style.background]="
                    block.kind === 'confirmed' ? 'var(--app-viz-2)' : 'var(--mat-sys-primary)'
                  "
