...
@Component({
  ...
  template: `
    ...
    <div
      class="grid grid-cols-7 gap-px text-xs rounded-lg overflow-hidden"
      style="background: var(--mat-sys-outline-variant)"
    >
      @for (d of monthDays(); track d.getTime()) {
        <button
          ...
          [style.background]="
            selected() && isSameDay(d, selected()!) ? 'var(--app-positive-bg)' : null
          "
          (click)="selected.set(d)"
        >
          <div
            class="font-bold"
            [style.color]="isSameDay(d, todayDate) ? 'var(--mat-sys-primary)' : null"
          >
            {{ d.getDate() }}
          </div>
          <div style="color: var(--mat-sys-on-surface-variant)">
            {{ t.dispatch.pickups }}{{ statsOf(d).pickups }} {{ t.dispatch.returns
            }}{{ statsOf(d).returns }}
          </div>
          <div style="color: var(--mat-sys-on-surface-variant)">
            {{ t.dispatch.available }} {{ statsOf(d).available }}
          </div>
        </button>
      }
    </div>
    @if (selected(); as sel) {
      <div class="mt-4">
        <h2 class="font-bold mb-2">
          {{ t.dispatch.dayDetail }}（{{ sel.getMonth() + 1 }}/{{ sel.getDate() }}）
        </h2>
            ...
            @for (b of dayBookings(sel); track b.id) {
              <li class="ui-card !p-3">
                ...
                {{ fmt(b.startTime) }} → {{ fmt(b.endTime) }}｜{{
                  t.booking.statusLabels[b.status]
                }}
              </li>
            }
          </ul>
